...
/**
 * @route POST /api/chat/cross-document-qa
 * @desc Ask questions across multiple documents with enhanced cross-document capabilities
 * @access Public
 */
router.post('/cross-document-qa', async (req, res) => {
  try {
    const { 
      sessionId, 
      question, 
    ...
    // Use enhanced cross-document Q&A
    const result = await documentService.askQuestionCrossDocument(question, sessionId, {
      useEnhancedAI: true,
      maxResults,
      confidenceThreshold,
    ...
    try {
      ...
      await chatHistoryService.addMessage(sessionId, {
        ...
        metadata: {
          crossDocumentQuery: true,
          documentsSearched: result.analysis?.documentsSearched || 0,
          sources: result.sources?.length || 0,
          responseType: 'cross-document-qa'
        },
        timestamp: new Date().toISOString()
      });
    } catch (historyError) {
    ...
    logger.error('Cross-document Q&A failed:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route GET /api/chat/session-documents/:sessionId
 * @desc Get all documents in a session for cross-document operations
 * @access Public
 */
router.get('/session-documents/:sessionId', async (req, res) => {
  try {
    ...
    const result = await documentService.getSessionDocuments(sessionId);
    
    res.json(result);
  } catch (error) {
    logger.error('Failed to get session documents:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route POST /api/chat/cross-document-search
 * @desc Search across multiple documents in a session
 * @access Public
 */
router.post('/cross-document-search', async (req, res) => {
  try {
    const { 
      sessionId, 
      query, 
    ...
    const result = await documentService.crossDocumentSearch(query, sessionId, {
      limit,
      confidenceThreshold,
      documentIds,
    ...
    logger.error('Cross-document search failed:', error);
    res.status(500).json({
      success: false,
